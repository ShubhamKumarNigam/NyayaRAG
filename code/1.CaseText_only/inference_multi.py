...
with open(output_csv_file, 'a', newline='', encoding='utf-8') as f:
    ...
    for i, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        prompt = f"""
        You are a legal expert tasked with making a judgment about whether an appeal should be accepted or rejected based on the provided summary of the case. Your task is to evaluate whether the appeal should be accepted (1) or rejected (0) based on the case proceedings provided below.

        ### Example 1:
        Case proceedings: "The defendant was convicted for assaulting a police officer during a protest. The incident was recorded on CCTV footage. Witnesses testified that the defendant acted in self-defense after being pushed by the officer."
        ##PREDICTION: 0
        ##EXPLANATION: The appeal is rejected because the defendant's actions were a direct response to police aggression, and there is no sufficient evidence to prove excessive use of force by the defendant.

        ### Example 2:
        Case proceedings: "The defendant was accused of embezzling funds from a charity. An audit revealed the misappropriation of large sums of money. The defendant was unable to provide any valid financial records to counter the audit findings."
        ##PREDICTION: 1
        ##EXPLANATION: The appeal is accepted because the defendant failed to provide credible evidence to support their claim, and the audit evidence strongly suggests fraudulent activity.
